...
const baselineSecurityHeaders = {
  "Content-Security-Policy": "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; media-src 'self' blob: https:; connect-src 'self' https://*.googleapis.com https://*.firebaseio.com wss://*.firebaseio.com https://*.firebaseapp.com; frame-src 'none'; worker-src 'self' blob:; manifest-src 'self'",
...
test("every Hosting target routes /api/v1/** to apiV1 before SPA rewrites", () => {
  ...
  for (const target of ["portal", "exam", "vendor"]) {
    ...
    const spaRewriteIndex = rewrites.findIndex((rewrite) =>
...
test("every Hosting target applies the identical baseline security headers", () => {
