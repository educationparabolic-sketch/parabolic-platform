...
const artifactDir = path.resolve("artifacts/build-145");
...
function buildExamToken(sessionId) {
  ...
  const payload = {
    ...
    mode: "Operational",
    instituteId: "inst-build-145",
    yearId: "2026",
    runId: "run-build-145",
    sessionId,
    refreshNonce: "build-145",
  };
  ...
  for (const viewport of viewports) {
    ...
    const scenarioChecks = [
      ...
      },
      {
        ...
        routeUrl: `${examBaseUrl}/session/session-build-145?token=${encodeURIComponent(buildExamToken("session-build-145"))}`,
        expectedPath: "/session/session-build-145",
