...
const artifactDir = path.resolve("artifacts/build-146");
...
function buildExamToken(sessionId) {
  ...
  const payload = {
    ...
    mode: "Operational",
    instituteId: "inst-build-146",
    yearId: "2026",
    runId: "run-build-146",
    sessionId,
    refreshNonce: "build-146",
  };
...
try {
  for (const viewport of viewports) {
    ...
    const scenarioChecks = [
      ...
      {
        routeId: "admin_login_legacy_to_overview_authenticated",
      ...
      },
      {
        ...
        routeUrl: `${examBaseUrl}/session/session-build-146?token=${encodeURIComponent(buildExamToken("session-build-146"))}`,
        expectedPath: "/session/session-build-146",
