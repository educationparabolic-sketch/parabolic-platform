...
const artifactDir = path.resolve("artifacts/build-149");
...
function buildExamToken(sessionId) {
  ...
  const payload = {
    ...
    mode: "Operational",
    instituteId: "inst-build-149",
    yearId: "2026",
    runId: "run-build-149",
    sessionId,
    refreshNonce: "build-149",
  };
...
try {
  for (const viewport of viewports) {
    ...
    const scenarioChecks = [
      {
        routeId: "admin_navigation_authenticated",
      ...
      {
        routeId: "student_dashboard_navigation",
      ...
      {
        routeId: "vendor_overview_role_guard",
      ...
      },
      {
        ...
        routeUrl: `${examBaseUrl}/session/session-build-149?token=${encodeURIComponent(buildExamToken("session-build-149"))}`,
        expectedPath: "/session/session-build-149",
