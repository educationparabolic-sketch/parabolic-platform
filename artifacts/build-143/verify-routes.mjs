...
const artifactDir = path.resolve("artifacts/build-143");
...
const adminBaseUrl = "http://127.0.0.1:4311";
const studentBaseUrl = "http://127.0.0.1:4312";
const vendorBaseUrl = "http://127.0.0.1:4313";
...
function buildExamToken(sessionId) {
  ...
  const payload = {
    ...
    mode: "Operational",
    instituteId: "inst-build-143",
    yearId: "2026",
    runId: "run-build-143",
    sessionId,
    refreshNonce: "build-143",
  };
...
const routeChecks = [
  ...
  {
    app: "vendor",
    route: "/vendor/system-health",
    url: `${vendorBaseUrl}/vendor/login`,
    expectedPath: "/vendor/system-health",
    expectedVisibleText: "Frontend Runtime Exceptions",
    guardExpected: "PASS",
    action: async (page) => {
      ...
      await page.goto(`${vendorBaseUrl}/vendor/system-health`, { waitUntil: "networkidle" });
      await page.waitForSelector("text=System Health Monitoring Dashboard");
    },
  },
  {
    app: "exam",
