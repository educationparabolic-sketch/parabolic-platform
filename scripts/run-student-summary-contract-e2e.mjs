...
function run(label, executable, args, extraEnvironment = {}) {
  console.log(`\n[student-summary-contract] ${label}`);
  ...
  if (result.error) {
    console.error(
      `[student-summary-contract] Unable to start ${label}: ` +
  ...
  if (result.status !== 0) {
    console.error(
      `[student-summary-contract] FAILED ${label} with exit code ` +
        `${result.status ?? 1}`,
    );
...
const liveBuildEnvironment = {
  ...
  VITE_FIREBASE_APP_ID: "1:123456789:web:bwm015summary",
...
run(
  "Build live Admin artifact for combined Portal Hosting",
  npmExecutable,
...
const emulatorArgs = [
  ...
  "npm run test:e2e:student-summary-contracts",
];

console.log(
  `\n[student-summary-contract] firebase ${emulatorArgs
