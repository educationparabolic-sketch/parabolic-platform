...
function run(label, executable, args, extraEnvironment = {}) {
  ...
  if (result.error) {
    console.error(`[admin-summary-contract] Unable to start ${label}: ${result.error.message}`);
    process.exit(1);
  }
  if (result.status !== 0) {
    console.error(`[admin-summary-contract] FAILED ${label} with exit code ${result.status ?? 1}`);
...
const liveBuildEnvironment = {
  ...
  VITE_FIREBASE_APP_ID: "1:123456789:web:bwm011summary",
...
run("Build live Admin artifact", npmExecutable, ["--prefix", "apps/admin", "run", "build"], liveBuildEnvironment);
...
const emulatorArgs = [
  ...
  "npm run test:e2e:admin-summary-contracts",
];

console.log(
  `\n[admin-summary-contract] firebase ${emulatorArgs
    .map((value) => (value.includes(" ") ? JSON.stringify(value) : value))
    .join(" ")}`,
);
run("Run seeded Admin summary browser flow", firebaseExecutable, emulatorArgs, {
  CI: "true",
...
console.log("\n[admin-summary-contract] PASS: seeded summary values and explicit failure state verified.");
