...
function run(label, executable, args, extraEnvironment = {}) {
  ...
  if (result.error) {
    console.error(`[admin-teacher-e2e] Unable to start ${label}: ${result.error.message}`);
    process.exit(1);
  }
  if (result.status !== 0) {
    console.error(`[admin-teacher-e2e] FAILED ${label} with exit code ${result.status ?? 1}`);
...
const liveBuildEnvironment = {
  ...
  VITE_FIREBASE_APP_ID: "1:123456789:web:bwm008teacher",
...
const emulatorArgs = [
  ...
  "npm run test:e2e:admin-teacher-access",
];

console.log(
  `\n[admin-teacher-e2e] firebase ${emulatorArgs
    .map((value) => (value.includes(" ") ? JSON.stringify(value) : value))
    .join(" ")}`,
);
run("Run teacher Admin browser flow", firebaseExecutable, emulatorArgs, {
  CI: "true",
...
console.log("\n[admin-teacher-e2e] PASS: teacher Admin route and handler alignment verified.");
