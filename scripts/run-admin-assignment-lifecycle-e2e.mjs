...
function run(label, executable, args, extraEnvironment = {}) {
  console.log(`\n[admin-assignment-lifecycle] ${label}`);
  ...
  if (result.error) {
    console.error(
      `[admin-assignment-lifecycle] Unable to start ${label}: ` +
  ...
  if (result.status !== 0) {
    console.error(
      `[admin-assignment-lifecycle] FAILED ${label} with exit code ` +
        `${result.status ?? 1}`,
    );
...
const liveBuildEnvironment = {
  ...
  VITE_FIREBASE_APP_ID: "1:123456789:web:bwm014lifecycle",
...
const emulatorArgs = [
  ...
  "npm run test:e2e:admin-assignment-lifecycle",
];

console.log(
  `\n[admin-assignment-lifecycle] firebase ${emulatorArgs
    .map((value) => value.includes(" ") ? JSON.stringify(value) : value)
    .join(" ")}`,
);
run(
  "Run no-mock Admin assignment lifecycle browser flow",
  firebaseExecutable,
  emulatorArgs,
  {
    CI: "true",
