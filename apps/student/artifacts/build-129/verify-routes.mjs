...
const outDir = "/home/sumeer/parabolic-platform/apps/student/artifacts/build-129";
...
const authenticatedRoute = {
  ...
  expectedText: "Student Performance Analytics",
  guardStatus: "PASS",
};
...
async function evaluatePage(page, routeConfig, viewportName, authState) {
  ...
  if (routeConfig.expectedPath !== routeConfig.route) {
    try {
      await page.waitForURL(`**${routeConfig.expectedPath}`, { timeout: 10000 });
    } catch {
      // Keep collecting deterministic evidence even when expected redirect fails.
    }
  }
...
async function loginAndOpenPerformance(page) {
  await page.goto(`${baseUrl}/student/performance`, { waitUntil: "networkidle" });
  ...
  await page.waitForURL("**/student/performance", { timeout: 15000 });
  await page.waitForTimeout(900);
}
...
for (const viewport of viewports) {
  ...
  try {
    ...
    await loginAndOpenPerformance(page);
