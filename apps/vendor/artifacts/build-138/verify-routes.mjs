...
const outDir = "/home/sumeer/parabolic-platform/apps/vendor/artifacts/build-138";
...
const vendorScenarios = [
  {
    route: "/vendor/calibration",
    expectedPath: "/vendor/calibration",
    expectedText: "Global Calibration Control",
    guardStatus: "PASS",
  },
  {
    route: "/vendor/audit",
    expectedPath: "/vendor/audit",
    expectedText: "Audit & Activity Logs",
    guardStatus: "PASS",
  },
];
...
async function evaluate(page, scenario) {
  ...
  if (scenario.expectedPath !== scenario.route) {
    try {
      ...
      // Deterministic failure evidence captured in report.
    }
  }
  await page.waitForTimeout(900);
...
for (const viewport of viewports) {
  ...
  for (const scenario of vendorScenarios) {
    results.push(
