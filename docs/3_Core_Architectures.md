...
Example license object:

  {
    ...
    License.currentLayer determines which pricing plan is used.
...
Billing calculations must resolve pricing from the vendor configuration.
...
invoiceAmount = basePriceMonthly + (activeStudents × pricePerStudent)

Where:

basePriceMonthly is the base institute subscription
pricePerStudent is variable billing based on active student count

This ensures pricing logic remains configurable without redeploying backend code.
