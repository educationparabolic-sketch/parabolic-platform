...
last_update_summary: BWM-010 is BLOCKED at its staging deployment substep. The approved apps/admin repair is published as commit 0e5b42811eb9d25b0c5c2953e7f79d0a34a10a33, and the product owner approved the adjacent shared dependency repair exposed by replacement run 32121022156. Backend CI now installs both locked dependency roots before the aggregate; four focused contracts, formatting, diff integrity, and the 126-module Admin production build pass locally. The staging GitHub Environment still does not exist, and the repository has only the authenticated owner as a collaborator, so the mandatory independent reviewer with self-review disabled cannot yet be configured. L5 remains unchecked until the shared repair is published and passes CI, an eligible reviewer and protected staging Environment plus required variables/FIREBASE_TOKEN are configured, and the manual workflow completes in Functions -> Firestore rules/indexes -> three Hosting targets order under Firebase CLI 15.9.0.
```
