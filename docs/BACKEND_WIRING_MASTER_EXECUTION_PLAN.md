...
last_update_summary: BWM-010 is BLOCKED at its staging deployment substep. The product owner approved the Java 21 runner repair exposed by CI run 32122353674. Backend CI now uses official actions/setup-java@v5 with Temurin 21, records the Java version, and contracts Java setup before pinned Firebase CLI 15.9.0 and the emulator aggregate. Four focused contract suites, formatting, and diff integrity pass locally. The staging GitHub Environment still does not exist, and the repository has only the authenticated owner as a collaborator. L5 remains unchecked until the Java repair is published and passes the full CI emulator/cleanup gate, an eligible reviewer and protected staging Environment plus required variables/FIREBASE_TOKEN are configured, and the manual workflow completes in Functions -> Firestore rules/indexes -> three Hosting targets order under Firebase CLI 15.9.0.
```
...
- **Fifth-substep in-progress evidence — ordered approval-gated staging pipeline (2026-08-18):**
  ...
  - **L3 Firebase emulator:** N/A for this bounded pipeline implementation — no handler, rule content, index content, trigger, emulator configuration, or Firebase runtime behavior changed. The immediately preceding BWM-010 aggregate remains green at 70/70 files, 217/217 integration assertions, and 2/2 Chromium checks under `demo-parabolic-test`.
  - **L4 browser E2E:** N/A for the local pipeline change — no UI or browser runtime changed. The required public staging browser/API checks belong to the next unchecked post-deploy smoke substep and cannot run before this deployment completes.
  - **L5 staging/preview:** BLOCKED / NOT RUN. The product owner explicitly authorized publishing to `staging` and deploying this release to `parabolic-dev` on 2026-08-18. GitHub CLI `2.45.0` is installed and authenticated as `educationparabolic-sketch`. Initial commit `d8f9a8ed1f7c07d026fb464330ffe29bb361cede` plus approved repair commits `0e5b42811eb9d25b0c5c2953e7f79d0a34a10a33`, `8586bc6d6fa16683556f38c5d1e9e7dd63d7dca6`, and `dcff0d48637cee68ae88feade16d4732eece519a` are published on remote `staging`. Runs `32120189655`, `32121022156`, and `32121695046` exposed and then cleared the complete Admin/shared/Student dependency closure. Run `32122353674` proved Admin (126), Student (100), Functions, and combined Portal Hosting builds pass, then Firebase CLI `15.9.0` rejected the runner's pre-21 Java before emulator service startup; cleanup confirmed all ports released. Every run passed complete frontend validation and correctly skipped `Staging Deploy`; no Firebase cloud command ran. The GitHub Environment `staging` also does not exist. The repository has only the authenticated owner as a collaborator, so an independent required reviewer with self-review disabled cannot yet be selected; Environment variables and `FIREBASE_TOKEN` are consequently absent. The checkbox remains unchecked and BWM-010 is `BLOCKED` rather than falsely marked complete.
