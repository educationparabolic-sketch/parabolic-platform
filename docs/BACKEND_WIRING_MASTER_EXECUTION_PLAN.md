...
current_substep: BWM-010 — Deploy Functions, Firestore rules/indexes, and Hosting to a dedicated staging Firebase project in an ordered, approval-gated pipeline
last_completed_task: BWM-009
next_task: BWM-010
blocked_tasks: [BWM-010]
last_updated: 2026-08-18
last_update_summary: BWM-010 is BLOCKED at its staging deployment substep. Owner-controlled Environment configuration and path-style Storage CDN repair commit 2d547a1dd2085806897a11292582228ab6f2251f are published. Replacement run 32125613065 passed frontend validation plus backend setup/lint/build/deterministic tests, then an intermittent UND_ERR_SOCKET occurred while deleting the disposable Auth smoke account after 2/2 Chromium checks; outer cleanup still cleared emulator data and released all ports, and deployment did not run. The product owner approved a cleanup-only repair that retries only UND_ERR_SOCKET/ECONNRESET up to three attempts and still fails HTTP errors, non-transient errors, or exhausted retries. Three focused contracts, syntax, diff integrity, and the complete local aggregate pass: 70 files/217 assertions, 2 Chromium checks, normal teardown, and intentional exit-23 teardown. L5 remains unchecked until this repair is published, replacement CI is green, and the authorized manual workflow completes Functions -> Firestore rules/indexes -> three Hosting targets order.
```
...
| BWM-010 | P0 | BLOCKED | BWM-001,BWM-004,BWM-005 | Backend CI and staging deploy pipeline |
...
- **Fourth-substep evidence — aggregate emulator integration gate and cleanup proof:**
  ...
  - **Completed on:** 2026-08-14
- **Fifth-substep in-progress evidence — ordered approval-gated staging pipeline (2026-08-18):**
  ...
  - **L5 staging/preview:** BLOCKED / NOT RUN. The product owner explicitly authorized publishing to `staging` and deploying this release to `parabolic-dev` on 2026-08-18. GitHub CLI `2.45.0` is installed and authenticated as `educationparabolic-sketch`. Owner-controlled Environment/path-style CDN repair commit `2d547a1dd2085806897a11292582228ab6f2251f` is published on remote `staging`. Run `32125613065` passed complete frontend validation and reached the full-service emulator phase, but the transient Auth cleanup socket failure stopped backend validation and correctly prevented `Staging Deploy`. On 2026-08-18 the product owner created the `staging` GitHub Environment, added `FIREBASE_TOKEN`, explicitly declined any independent reviewer, and retained the manual owner-controlled gate. Read-only API verification saw only the secret name. The Environment has a custom deployment branch policy matching only `staging` and 15 public variables sourced from authenticated Firebase web-app/Hosting metadata and the verified default Storage bucket. No Firebase deployment command has run yet. The checkbox remains unchecked until the approved cleanup retry is published, passes replacement CI, and the manual deployment succeeds.
  ...
  - **Firebase CLI version:** Local and Actions runner `firebase --version` both printed exact `15.9.0`; the workflow install and contract pin that version. The runner logged Temurin OpenJDK `21.0.12` before the CLI version and aggregate. No Firebase project or deployment command ran.
  - **Authorization/external mutations:** The product owner explicitly authorized committing/pushing these changes to `staging` and deploying Functions, Firestore rules/indexes, and Hosting to `parabolic-dev` on 2026-08-18. GitHub CLI `2.45.0` was installed from Ubuntu's signed `noble-updates` package into user-local `~/.local/bin`, and browser/device authentication stored the `educationparabolic-sketch` login in the user's GitHub CLI configuration; the existing SSH key was deliberately not uploaded. Commits through `2d547a1dd2085806897a11292582228ab6f2251f` are published on remote `staging`; build-only Actions run `32125613065` failed closed and did not enter its manual-dispatch-only deploy job. The product owner created Environment `staging` and supplied `FIREBASE_TOKEN` through GitHub's secret UI without disclosing it. After the owner rejected an independent reviewer, the Environment was changed only to add an exact custom `staging` branch policy and 15 verified non-secret variables; the secret value was never read or logged. Authenticated read-only Firebase app, SDK config, Hosting site/run/log inventories, HTTPS endpoint probes, and approved local emulators ran. No Firebase project, Function, rule, index, Hosting release, remote data, secret value, public URL, or production resource changed.
  ...
  - **Residual risks/blocker:** Repository-side dependency closure, Java 21 setup, Environment creation, branch restriction, secret-name presence, public configuration, and local aggregate/cleanup proof are complete. Publish the approved cleanup-only retry and require replacement CI to remain green, then dispatch the already-authorized manual workflow. After dispatch, capture the exact Firebase CLI output, Functions/rules/index/Hosting release identifiers, and exit results here. `FIREBASE_TOKEN` is the Firebase-documented legacy CI mechanism and must later migrate to Application Default Credentials before Firebase removes token support. The default staging bucket temporarily supplies both logical question-asset and report bucket variables; owning asset/storage tasks must provision narrower infrastructure if their verified flows require it. Post-deploy API health/authenticated smoke and production-promotion separation remain the following unchecked BWM-010 substeps.
