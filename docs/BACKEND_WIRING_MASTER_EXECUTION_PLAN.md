...
last_update_summary: BWM-010 is BLOCKED at its staging deployment substep. GitHub CLI 2.45.0 is installed and authenticated as educationparabolic-sketch, and the product owner explicitly authorized publishing to staging and mutating parabolic-dev on 2026-08-18. Pre-publish inspection found that neither the remote staging branch nor the staging GitHub Environment exists and that the repository has only the authenticated owner as a collaborator, so the mandatory independent required reviewer with self-review disabled cannot yet be configured. L5 remains unchecked until the bounded changes are published, an eligible reviewer and protected staging Environment plus required variables/FIREBASE_TOKEN are configured, and the approved workflow completes in Functions -> Firestore rules/indexes -> three Hosting targets order under Firebase CLI 15.9.0.
```
...
- **Fifth-substep in-progress evidence — ordered approval-gated staging pipeline (2026-08-18):**
  ...
  - **L5 staging/preview:** BLOCKED / NOT RUN. The product owner explicitly authorized publishing to `staging` and deploying this release to `parabolic-dev` on 2026-08-18. GitHub CLI `2.45.0` is now installed and authenticated as `educationparabolic-sketch`. Read-only pre-publish checks confirmed the correct public repository and default `main` branch, but neither remote branch `staging` nor GitHub Environment `staging` exists. The repository has only the authenticated owner as a collaborator, so an independent required reviewer with self-review disabled cannot yet be selected; the Environment variables and `FIREBASE_TOKEN` are consequently also absent. No cloud command or GitHub workflow was dispatched. The checkbox remains unchecked and BWM-010 is `BLOCKED` rather than falsely marked complete.
  ...
  - **Authorization/external mutations:** The product owner explicitly authorized committing/pushing these changes to `staging` and deploying Functions, Firestore rules/indexes, and Hosting to `parabolic-dev` on 2026-08-18. GitHub CLI `2.45.0` was installed from Ubuntu's signed `noble-updates` package into user-local `~/.local/bin`, and browser/device authentication stored the `educationparabolic-sketch` login in the user's GitHub CLI configuration; the existing SSH key was deliberately not uploaded. Read-only GitHub repository, branch, Environment, variable/secret-name, and collaborator checks followed. Official Firebase documentation was rechecked and confirms `--only firestore` deploys both configured Firestore rules and indexes and project-specific Functions dotenv files supply deployed runtime environment values. GitHub documentation confirms jobs referencing an Environment wait for configured required-reviewer protection before running or accessing Environment secrets. No workflow, branch, commit, push, repository setting, Firebase project, Function, rule, index, Hosting release, remote data, secret, or public URL changed.
