...
last_update_summary: BWM-010 is BLOCKED at its staging deployment substep. The product owner explicitly declined an independent reviewer and selected owner-controlled safeguards: manual dispatch, exact parabolic-dev confirmation, staging-only Environment branch policy, environment-scoped credential, and ordered fail-fast deployment. The staging GitHub Environment now exists, its FIREBASE_TOKEN name is present without exposing its value, its custom deployment branch policy allows only staging, and all 15 required public variables use authenticated Firebase/Hosting metadata plus the verified parabolic-dev Storage bucket. A scoped validator repair now permits an environment-owned path in the matching CDN base URLs while continuing to forbid credentials, queries, fragments, HTTP, and paths on portal/API origins; four focused contract suites and diff integrity pass locally. L5 remains unchecked until this repair is published, replacement CI is green, and the authorized manual workflow completes Functions -> Firestore rules/indexes -> three Hosting targets order.
```
...
- **Fifth-substep in-progress evidence — ordered approval-gated staging pipeline (2026-08-18):**
  ...
  - **L3 Firebase emulator:** Actions run `32122997572` — PASS under explicit `demo-parabolic-test`. Temurin `21.0.12` and Firebase CLI `15.9.0` started the accumulated gate successfully. The five-service phase passed all 8 explicit suites and 19 assertions, the Firestore-only phase reported 62 files and 198 passing assertions, and the aggregate reported success after complete teardown: 70/70 files and 217/217 assertions overall. The intentional-failure child exited `23`; its outer proof passed only after disposable data cleanup, emulator shutdown, and release of all ten emulator/support ports. No handler, rule content, index content, trigger, emulator configuration, or Firebase runtime behavior changed.
  ...
  - **L5 staging/preview:** BLOCKED / NOT RUN. The product owner explicitly authorized publishing to `staging` and deploying this release to `parabolic-dev` on 2026-08-18. GitHub CLI `2.45.0` is installed and authenticated as `educationparabolic-sketch`. Commits through Java 21 repair `94416ea224adbda48115f3a40589596406b87701` and checkpoint `18e881ae0d88c2195228b908b9b4bca35472d25f` are published on remote `staging`. Replacement run `32122997572` passed complete frontend validation in 1m59s and backend validation in 3m42s, including Admin (126), Student (100), Functions, combined Portal Hosting, Temurin `21.0.12`, pinned Firebase CLI `15.9.0`, the complete emulator aggregate, and intentional-failure cleanup. On 2026-08-18 the product owner created the `staging` GitHub Environment, added `FIREBASE_TOKEN`, explicitly declined any independent reviewer, and retained the manual owner-controlled gate. Read-only API verification saw only the secret name. The Environment now has a custom deployment branch policy matching only `staging` and 15 public variables sourced from authenticated Firebase web-app/Hosting metadata and the verified default Storage bucket. The exact path-style bucket endpoint returned HTTPS 403 for an unauthenticated bucket-root listing, proving TLS/routing without reading data; the invalid bucket-name origin did not resolve and its virtual-host alternative failed certificate validation. No Firebase cloud command has run yet. The checkbox remains unchecked until the scoped path-style CDN validation repair is published, passes replacement CI, and the manual deployment succeeds.
