...
last_update_summary: BWM-010 is BLOCKED at its staging deployment substep. Approved Java 21 repair commit 94416ea224adbda48115f3a40589596406b87701 is published on staging, and Actions run 32122997572 passed complete frontend/backend validation with Temurin 21.0.12, Firebase CLI 15.9.0, all 70 emulator files and 217 assertions, 2 Chromium checks, and intentional-failure cleanup with every port released. The repository-side pipeline is green and its manual deploy job correctly skipped on push. The staging GitHub Environment still does not exist, and the repository has only the authenticated owner as a collaborator. L5 and the deployment checkbox remain blocked until an eligible independent reviewer, protected staging Environment, required variables, and FIREBASE_TOKEN are configured and the authorized manual workflow completes in Functions -> Firestore rules/indexes -> three Hosting targets order.
```
...
- **Fifth-substep in-progress evidence — ordered approval-gated staging pipeline (2026-08-18):**
  ...
  - **L5 staging/preview:** BLOCKED / NOT RUN. The product owner explicitly authorized publishing to `staging` and deploying this release to `parabolic-dev` on 2026-08-18. GitHub CLI `2.45.0` is installed and authenticated as `educationparabolic-sketch`. Initial commit `d8f9a8ed1f7c07d026fb464330ffe29bb361cede` plus approved repair commits `0e5b42811eb9d25b0c5c2953e7f79d0a34a10a33`, `8586bc6d6fa16683556f38c5d1e9e7dd63d7dca6`, `dcff0d48637cee68ae88feade16d4732eece519a`, and Java 21 commit `94416ea224adbda48115f3a40589596406b87701` are published on remote `staging`. Runs `32120189655`, `32121022156`, `32121695046`, and `32122353674` exposed and then cleared the complete Admin/shared/Student dependency closure and Java prerequisite. Replacement run `32122997572` passed complete frontend validation in 1m59s and backend validation in 3m42s, including Admin (126), Student (100), Functions, combined Portal Hosting, Temurin `21.0.12`, pinned Firebase CLI `15.9.0`, the complete emulator aggregate, and intentional-failure cleanup. Its `Staging Deploy` job correctly skipped because a branch push cannot trigger the manual deployment; no Firebase cloud command ran. The GitHub Environment `staging` does not exist. The repository has only the authenticated owner as a collaborator, so an independent required reviewer with self-review disabled cannot yet be selected; Environment variables and `FIREBASE_TOKEN` are consequently absent. The checkbox remains unchecked and BWM-010 is `BLOCKED` rather than falsely marked complete.
  ...
  - **Authorization/external mutations:** The product owner explicitly authorized committing/pushing these changes to `staging` and deploying Functions, Firestore rules/indexes, and Hosting to `parabolic-dev` on 2026-08-18. GitHub CLI `2.45.0` was installed from Ubuntu's signed `noble-updates` package into user-local `~/.local/bin`, and browser/device authentication stored the `educationparabolic-sketch` login in the user's GitHub CLI configuration; the existing SSH key was deliberately not uploaded. Commits through `94416ea224adbda48115f3a40589596406b87701` are published on remote `staging`; build-only Actions run `32122997572` passed and did not enter its manual-dispatch-only deploy job. Read-only GitHub repository, Environment, variable/secret-name, collaborator, run, and log checks also ran. Official Firebase documentation was rechecked and confirms `--only firestore` deploys both configured Firestore rules and indexes and project-specific Functions dotenv files supply deployed runtime environment values. GitHub documentation confirms jobs referencing an Environment wait for configured required-reviewer protection before running or accessing Environment secrets. No repository Environment/secret/variable, Firebase project, Function, rule, index, Hosting release, remote data, secret value, or public URL changed.
  - **Contract/schema changes:** Staging deployment is now an explicit manual, typed-project-confirmed, protected-Environment operation and production promotion is absent. The deployable Functions runtime environment is a validated 11-key non-secret project-specific dotenv generated only inside the approved staging job. API routes/DTOs, authentication/authorization, Firestore rules/index definitions, persistence schema, and application behavior are unchanged.
  - **Residual risks/blocker:** Repository-side dependency closure, Java 21 setup, full CI aggregate, and intentional-failure cleanup are complete and proven. Add at least one eligible independent repository collaborator or team reviewer, create the `staging` GitHub Environment with that required reviewer and self-review disabled, restrict it to branch `staging`, and populate every matrix-required `var` plus `FIREBASE_TOKEN`. Do not silently substitute owner self-approval for the documented independent gate. The product-owner authorization naming `parabolic-dev` is recorded and does not need to be inferred. After dispatch, capture the exact Firebase CLI output, Functions/rules/index/Hosting release identifiers, and exit results here. Post-deploy API health/authenticated smoke and production-promotion separation remain the following unchecked BWM-010 substeps.
