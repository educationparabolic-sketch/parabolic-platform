...
POST /vendor/license/update | Build 93 | Update the authoritative institute license through a vendor-only endpoint that validates requested layer and billing-plan inputs against vendor pricing configuration, writes `institutes/{instituteId}/license/current`, and mirrors the payload to `license/main` for backward compatibility with earlier backend modules
