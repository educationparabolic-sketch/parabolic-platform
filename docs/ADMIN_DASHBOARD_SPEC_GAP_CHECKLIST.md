...
| GBL-004 | Global | Full live API/data wiring replacing fixture-backed masking in production-like admin flows | partial | P0 | `/admin/analytics`, `/admin/analytics/trends`, `/admin/analytics/template/:testId`, `/admin/tests`, `/admin/tests/analytics/:testId`, `/admin/tests/:testId`, `/admin/assignments`, `/admin/assignments/create`, `/admin/assignments/list`, `/admin/assignments/history`, `/admin/assignments/bulk`, `/admin/assignments/live`, the focused run summary on `/admin/assignments/live/:runId`, `/admin/settings`, `/admin/insights`, and `/admin/students` now hydrate from live admin APIs with deterministic local fallback; recipient and batch selection on `/admin/assignments/create` now also hydrate from `GET /admin/students`; several other admin modules remain partially fixture-backed |
