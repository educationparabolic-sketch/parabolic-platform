...
| GBL-004 | Global | Full live API/data wiring replacing fixture-backed masking in production-like admin flows | partial | P0 | `/admin/analytics`, `/admin/analytics/trends`, `/admin/analytics/template/:testId`, `/admin/tests`, `/admin/tests/analytics/:testId`, `/admin/tests/:testId`, `/admin/assignments`, `/admin/assignments/create`, `/admin/assignments/list`, `/admin/assignments/history`, `/admin/assignments/bulk`, `/admin/assignments/live`, and `/admin/settings`, `/admin/insights`, and `/admin/students` now hydrate from live admin APIs with deterministic local fallback; `/admin/students/:studentId` now keeps its dedicated profile workspace on `GET /admin/students` in live mode while deterministically backfilling missing deep profile panels like history, trends, override summaries, and last-active data from the matched local fixture shape instead of letting sparse live payloads collapse that route back into empty charts/tables; `/admin/assignments/live/:runId` now hydrates both its focused run summary and its per-student live monitor rows from `GET /admin/analytics` in live mode instead of leaving the student table fixture-backed; recipient and batch selection on `/admin/assignments/create` now also hydrate from `GET /admin/students`; `/admin/question-bank/upload-package` now preserves local ZIP/workbook/image validation while adding live validate-only ingestion checks through `POST /admin/questions/bulk` with exam-subject context, `/admin/question-bank` now hydrates its landing summary from `GET /admin/questions/library`, `GET /admin/questions/distribution`, and `GET /admin/questions/upload-logs`, `/admin/question-bank/library` now hydrates persisted question records from `GET /admin/questions/library`, `/admin/question-bank/tags` now derives its governed tag inventory and protection signals from that same `GET /admin/questions/library` payload in live mode, `/admin/question-bank/archive` now derives archive/version lifecycle summaries from that same `GET /admin/questions/library` payload in live mode, `/admin/question-bank/validation-logs` now hydrates immutable upload summaries from `GET /admin/questions/upload-logs`, `/admin/question-bank/distribution` now hydrates precomputed difficulty, marks, chapter, and L2 risk summaries from `GET /admin/questions/distribution`, and `/admin/licensing/*` now has a real `POST /admin/licensing` snapshot backend that reads institute license, usage-meter, and immutable license-history data instead of leaving live-mode reads pointed at a missing endpoint; several other admin modules remain partially fixture-backed |
