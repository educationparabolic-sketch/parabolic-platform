...
| GBL-004 | Global | Full live API/data wiring replacing fixture-backed masking in production-like admin flows | partial | P0 | `/admin/analytics`, `/admin/analytics/trends`, `/admin/analytics/template/:testId`, `/admin/tests`, `/admin/tests/analytics/:testId`, `/admin/tests/:testId`, `/admin/assignments`, `/admin/assignments/create`, `/admin/assignments/list`, `/admin/assignments/history`, `/admin/assignments/bulk`, `/admin/assignments/live`, and `/admin/settings`, `/admin/insights`, and `/admin/students` now hydrate from live admin APIs with deterministic local fallback; `/admin/settings/*` now resolves its `POST /admin/settings` reads and mutations from the authenticated tenant in session claims instead of routing those production-like settings workspaces through the old env/demo institute ID fallback; `/admin/students/:studentId` now keeps its dedicated profile workspace on `GET /admin/students` in live mode while deterministically backfilling missing deep profile panels like history, trends, override summaries, and last-active data from the matched local fixture shape instead of letting sparse live payloads collapse that route back into empty charts/tables; `/admin/insights/student/:studentId` now resolves its intervention-history reads from the authenticated tenant in session claims instead of sending that dedicated student-intelligence workspace through a stale hard-coded institute fallback before calling the shared intervention API; `/admin/assignments/live/:runId` now hydrates both its focused run summary and its per-student live monitor rows from `GET /admin/analytics` in live mode instead of leaving the student table fixture-backed; recipient and batch selection on `/admin/assignments/create` now also hydrate from `GET /admin/students`; `/admin/tests/create` now hydrates its question-pool selection from `GET /admin/questions/library` in live mode with deterministic local fallback instead of sourcing that structural selection step exclusively from fixture-era local question bank data; insights workspace navigation and landing deep links now resolve `/admin/insights/student/:studentId` from the currently hydrated tenant student summaries or intervention queue instead of hard-coding the fallback fixture student ID; `/admin/insights/interventions` now resolves its `POST /admin/interventions` reads and writes against the authenticated tenant plus the currently loaded academic-year summary instead of a stale hard-coded institute/year pair; `/admin/governance/*` now resolves `POST /admin/governance/snapshots` against the authenticated tenant plus the active academic year from secured settings state instead of a stale demo institute/year pair; `/admin/question-bank/upload-package` now preserves local ZIP/workbook/image validation while adding live validate-only ingestion checks through `POST /admin/questions/bulk` with exam-subject context, `/admin/question-bank` now hydrates its landing summary from `GET /admin/questions/library`, `GET /admin/questions/distribution`, and `GET /admin/questions/upload-logs`, `/admin/question-bank/library` now hydrates persisted question records from `GET /admin/questions/library`, `/admin/question-bank/tags` now hydrates and mutates governed tag inventory through `GET /admin/questions/tags` and `POST /admin/questions/tags`, persisting create/rename/merge/deprecate operations in institute `tagDictionary` metadata while rewriting affected `questionBank` tag references for rename/merge actions in live mode, `/admin/question-bank/archive` now derives archive/version lifecycle summaries from that same `GET /admin/questions/library` payload in live mode, `/admin/question-bank/validation-logs` now hydrates immutable upload summaries from `GET /admin/questions/upload-logs`, `/admin/question-bank/distribution` now hydrates precomputed difficulty, marks, chapter, and L2 risk summaries from `GET /admin/questions/distribution`, and `/admin/licensing/*` now resolves its `POST /admin/licensing` snapshot reads from the authenticated tenant in session claims instead of pinning those production-like licensing workspaces to the old env/demo institute ID fallback; several other admin modules remain partially fixture-backed |
