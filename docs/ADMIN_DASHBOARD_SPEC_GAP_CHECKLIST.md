...
| GBL-004 | Global | Full live API/data wiring replacing fixture-backed masking in production-like admin flows | partial | P0 | Core admin P0 live backend coverage now includes secured `GET /admin/overview`, `GET /admin/analytics`, `GET`/`POST /admin/tests`, `GET /admin/students`, and `POST /admin/runs`; `/admin/assignments/create` now has backend scheduling that resolves the authenticated tenant active year, persists scheduled run snapshots, and delegates template, recipient, license, and immutable snapshot validation to the assignment creation service. The remaining P0-sized GBL-004 gap is making `/admin/students/:studentId` deep profile panels truly live for history, risk/discipline trends, override summaries, and last-active data instead of deterministic fixture backfill. |
